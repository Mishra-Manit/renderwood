...
vi.mock("@pydantic/logfire-node", () => ({
  ...
  error: vi.fn(),
  debug: vi.fn(),
  span: vi.fn((_name: string, opts: any) => {
    if (opts && typeof opts.callback === "function") return opts.callback();
    return undefined;
  }),
  startSpan: vi.fn(),
  reportError: vi.fn(),
...
describe("orchestrator", () => {
  ...
  it("throws if job directory already exists", async () => {
    ...
    await expect(run("dup-job", "test", VideoStyle.GENERAL)).rejects.toThrow(
      /already exists/,
    );
  });
