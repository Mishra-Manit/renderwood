...
function isHiddenOrSidecar(dir: string, name: string): boolean {
...
export function collectAssetSummaries(): AssetSummary[] {
  ...
  for (const name of entries) {
    ...
    if (!stat?.isFile() || isHiddenOrSidecar(dir, name)) continue;
...
export function formatAssetsContext(summaries: AssetSummary[]): string {
...
export function copyUploadsToJob(jobDir: string): void {
  const dir = config.uploadDir;
  if (!existsSync(dir)) return;
