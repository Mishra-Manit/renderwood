...
function thumbDir(uploadDir: string): string {
...
function readMetadata(filePath: string): Record<string, unknown> | null {
...
function hasThumbnail(filePath: string, meta: Record<string, unknown> | null): boolean {
  if (!meta) return false;
  const tn = meta.thumbnail_name as string;
  if (!tn) return false;
  return existsSync(join(thumbDir(join(filePath, "..")), basename(tn)));
}

function generateVideoThumbnail(filePath: string, mimeType: string): string {
...
export async function uploadRoutes(app: FastifyInstance) {
  app.get("/api/uploads", async () => {
    ...
    for (const name of entries) {
      ...
      if (!stat?.isFile() || name.startsWith(".") || isSidecar(fp)) continue;
  ...
  app.get<{ Params: { filename: string } }>("/api/uploads/:filename/thumbnail", async (request, reply) => {
    const safeName = basename(request.params.filename);
    const fp = join(config.uploadDir, safeName);
