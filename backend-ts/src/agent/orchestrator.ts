...
import {
  collectAssetSummaries,
  formatAssetsContext,
  copyUploadsToJob,
} from "../lib/upload-assets.js";
import { VideoStyle } from "./video-styles.js";

export async function run(
  jobId: string,
  prompt: string,
  videoStyle: VideoStyle = VideoStyle.GENERAL,
): Promise<{ output_path: string; job_project_path: string }> {
  return withSpan(
    "remotion_video_generation",
    { job_id: jobId, user_prompt: prompt, video_style: videoStyle },
    async () => {
...
function setupJobDirectory(jobId: string): {
  jobDir: string;
  outputDir: string;
} {
...
function buildAgentOptions(jobDir: string): Options {
  const opts: Options = {
  ...
  try {
    for await (const message of conversation) {
      if (message.type === "assistant") {
        ...
        traceResult(message as SDKResultMessage, turnCount);
        handleResultMessage(
...
function traceAssistantTurn(
  ...
  const toolCalls = content
    ...
    .map((b: { type: string; name: string }) => b.name);
...
function handleResultMessage(
  ...
  if (message.is_error) {
    ...
    logfire.warning("agent_error_after_render", {
      message:
        "Agent reported an error but the video was already rendered. Continuing.",
      error: errorText,
    });
    return;
  }
...
function copyOutputToFinal(outputPath: string, jobId: string): string {
